  ...
  <div class="idx" style="grid-column:1/-1">
    ...
    <div style="margin-top:10px;display:grid;grid-template-columns:repeat(4,1fr);gap:8px">
      ...
      <div style="background:rgba(248,113,113,0.06);border:1px solid rgba(248,113,113,0.25);border-radius:var(--radius-sm);padding:14px;text-align:center">
        ...
        <div style="font-size:0.56rem;color:var(--muted);margin-top:5px;line-height:1.7">Worse than Bitcoin (85) or any nation on Earth (max 63). Protocol at maximum intervention. Wealth cap at 25× mean enforced.</div>
      </div>
    </div>
...
zh:{
  ...
  'p0':'引导期 · 不足100人 · 财富上限：25× 平均余额 · 当前激活',
  ...
  'ubi-src-cap':'财富上限溢出','ubi-src-cap-d':'超过25×平均余额的钱包立即被没收超额部分。20%立即流入UBI。',
...
function _buildNodeGuidePDF(lang) {
  ...
  var C={
    en:{title:'Aequitas Node Operator Guide',sub:'Complete step-by-step guide · Aequitas Chain (Chain ID 1926)',badge:'BETA v0.1 · Open Source · Permissionless · No stake required',
      ...
      s9:'9. Troubleshooting',th:['Symptom','Likely Cause','Fix'],tr:[['Height stays at 0 after start','PEER_NODES not set or unreachable','Set PEER_NODES=https://aequitas-production-9fba.up.railway.app and redeploy'],['"no code at address" in logs','V7 contract not deployed in EVM','Verify RELAYER_ADDRESS is set; node auto-deploys V7 on startup if missing'],['"NODE_OPERATOR_WALLET not set" in logs','Missing env var','Set NODE_OPERATOR_WALLET to your registered human wallet address'],['DATABASE_URL error on startup','Wrong connection string or DB unreachable','Check format: postgres://user:pass@host:5432/dbname and that PostgreSQL is running'],['Port 8080 not reachable','Firewall or cloud provider config','Open TCP 8080 inbound; check Railway/Render/VPS port settings'],['Docker build fails with go module error','No internet access during build','Ensure Docker build environment has outbound internet; Railway handles this automatically']],
      s10:'10. MetaMask Configuration',m10:'To use your own node as the RPC endpoint in MetaMask or any EVM-compatible wallet:',mh:['Field','Value'],mr:[['Network Name','Aequitas Chain'],['RPC URL','https://YOUR-NODE-URL/rpc'],['Chain ID','1926  (hex: 0x786)'],['Currency Symbol','AEQ'],['Decimals','18'],['Block Explorer','https://aequitas-production-9fba.up.railway.app']],
