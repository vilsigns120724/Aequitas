...
<div class="nc" style="grid-column:1/-1;background:linear-gradient(135deg,rgba(245,166,35,0.06),rgba(13,8,32,0.9));border-color:rgba(245,166,35,0.2)">
    ...
    <div id="node-guide" style="display:block;background:var(--card);border:1px solid rgba(245,166,35,0.2);border-radius:var(--radius);padding:24px;margin-top:4px">
      ...
      <div style="font-size:0.62rem;color:var(--muted);line-height:1.9;margin-bottom:10px">Instead of a shared PEER_SECRET, register your node signing key with your human wallet. Requires two signatures to prove you control both keys. The endpoint is public — call it from <strong>any machine</strong> (your laptop, another server) using your node's public URL:</div>
      <div style="font-family:var(--font-mono);background:rgba(0,0,0,0.35);border:1px solid rgba(139,92,246,0.15);border-radius:6px;padding:10px 14px;font-size:0.6rem;color:var(--teal);margin-bottom:12px;overflow-x:auto">curl "https://<span style="color:var(--gold)">YOUR-NODE-URL</span>/api/sign-validator-challenge?wallet=<span style="color:var(--gold)">0xYOUR_HUMAN_WALLET</span>"<br><br><span style="color:var(--muted)"># Railway example:</span><br>curl "https://aequitas-production-f0d2.up.railway.app/api/sign-validator-challenge?wallet=<span style="color:var(--gold)">0xYOUR_HUMAN_WALLET</span>"</div>
...
function _buildNodeGuidePDF(lang) {
  ...
  var C={
    ...
    de:{title:'Aequitas Node-Betreiber-Handbuch',sub:'Vollständige Schritt-für-Schritt-Anleitung · Aequitas Chain (Chain ID 1926)',badge:'v1.0 · Open Source · Erlaubnisfrei · Kein Stake erforderlich',
      ...
      s8b:'8b. Validator-Key registrieren (dezentral)',w8b:'Statt PEER_SECRET: Signing-Key mit Human-Wallet verknüpfen. Der Primär-Node akzeptiert dann deine Blöcke automatisch.',b8b:['Von jedem beliebigen Rechner (Laptop, Heim-PC) ausführen: curl "https://DEIN-NODE-URL/api/sign-validator-challenge?wallet=0xDEINE_HUMAN_WALLET" — DEIN-NODE-URL ersetzen durch deine Railway/Render-URL (z.B. aequitas-production-f0d2.up.railway.app). Kein SSH nötig — der Endpoint ist öffentlich erreichbar.','Website aufrufen: Network-Tab, Node Guide, zu Schritt 5b scrollen','RELAYER_ADDRESS und die Signatur aus dem curl-Output eintragen','"Sign with MetaMask" mit der registrierten Human-Wallet klicken','Primär-Node akzeptiert Blöcke sofort; PRIMARY_NODE_URL=https://aequitas.digital setzen für Auto-Discovery'],
    ...
    es:{title:'Guia del Operador de Nodos Aequitas',sub:'Guia completa paso a paso · Aequitas Chain (Chain ID 1926)',badge:'v1.0 · Codigo Abierto · Sin permisos · Sin stake requerido',
      ...
      s9:'9. Solucion de Problemas',th:['Sintoma','Causa probable','Solucion'],tr:[['Altura permanece en 0','PEER_NODES no configurado','Establece PRIMARY_NODE_URL=https://aequitas.digital y redespliega'],['"no code at address" en logs','Contrato V7 no desplegado aun','Normal en el primer inicio — el nodo despliega V7 automaticamente'],['Error DATABASE_URL','Cadena de conexion incorrecta','Verifica el formato: postgres://usuario:clave@host:5432/dbname'],['Puerto 8080 no accesible','Firewall o configuracion del proveedor','Abre TCP 8080 entrante en tu firewall']],
    ...
    pt:{title:'Guia do Operador de Node Aequitas',sub:'Guia completo passo a passo · Aequitas Chain (Chain ID 1926)',badge:'v1.0 · Open Source · Sem permissao · Sem stake necessario',
      ...
      s9:'9. Resolucao de Problemas',th:['Sintoma','Causa provavel','Solucao'],tr:[['Altura fica em 0','PRIMARY_NODE_URL nao definida','Definir PRIMARY_NODE_URL=https://aequitas.digital e reimplantar'],['"no code at address" nos logs','Contrato V7 ainda nao implantado','Normal na primeira vez — node implanta V7 automaticamente'],['Erro DATABASE_URL','String de conexao incorreta','Verificar formato: postgres://user:pass@host:5432/dbname'],['Porta 8080 inacessivel','Firewall ou configuracao do provedor','Abrir TCP 8080 inbound no firewall']],
