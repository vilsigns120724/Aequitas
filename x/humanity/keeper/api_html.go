...
<div class="nc" style="grid-column:1/-1;background:linear-gradient(135deg,rgba(245,166,35,0.06),rgba(13,8,32,0.9));border-color:rgba(245,166,35,0.2)">
    ...
    <div id="node-guide" style="display:block;background:var(--card);border:1px solid rgba(245,166,35,0.2);border-radius:var(--radius);padding:24px;margin-top:4px">
      ...
      <div style="font-family:var(--font-mono);background:rgba(0,0,0,0.3);border:1px solid rgba(139,92,246,0.15);border-radius:6px;padding:14px;margin-bottom:8px;font-size:0.61rem;line-height:2.2;overflow-x:auto">
        ...
        <span style="color:var(--muted)"># 2. First start — no NODE_KEY yet (it will be printed in logs)</span><br>
        ...
        &nbsp;&nbsp;-e RELAYER_ADDRESS="0xYOUR_NODE_WALLET" \<br>
        &nbsp;&nbsp;-e NODE_OPERATOR_WALLET="0xYOUR_HUMAN_WALLET" \<br>
        &nbsp;&nbsp;-e PEER_SECRET="get-from-network-operator" \<br>
        &nbsp;&nbsp;-e SELF_URL="http://YOUR-SERVER-IP:8080" \<br>
        &nbsp;&nbsp;-e PRIMARY_NODE_URL="https://aequitas.digital" \<br>
        &nbsp;&nbsp;-e BOOTSTRAP_SNAPSHOT_URL="https://aequitas.digital/api/snapshot" \<br>
        &nbsp;&nbsp;-e BOOTSTRAP_SIGNER="0x92cbedec9d348b4762cb9af99500ee6139c5b671" \<br>
        &nbsp;&nbsp;-e SNAPSHOT_TOKEN="get-from-network-operator" \<br>
        &nbsp;&nbsp;-p 8080:8080 -p 4001:4001 \<br>
        &nbsp;&nbsp;aequitas-node<br><br>
        <span style="color:var(--muted)"># 3. Copy NODE_KEY from logs (appears on first start):</span><br>
        docker logs aequitas-node 2&gt;&amp;1 | grep "SAVE THIS AS NODE_KEY"<br><br>
        <span style="color:var(--muted)"># 4. Restart with NODE_KEY to keep stable Peer ID across restarts:</span><br>
        docker stop aequitas-node &amp;&amp; docker rm aequitas-node<br>
        docker run -d --name aequitas-node --restart unless-stopped \<br>
        &nbsp;&nbsp;-e NODE_KEY="&lt;base64 from step 3&gt;" \<br>
...
function _buildNodeGuidePDF(lang) {
  ...
  var C={
    en:{title:'Aequitas Node Operator Guide',sub:'Complete step-by-step guide · Aequitas Chain (Chain ID 1926)',badge:'v1.0 · Open Source · Permissionless · No stake required',
      ...
      s5:'5. Quick Start — Docker / VPS',d5:'For VPS or cloud VM (Contabo, Hetzner, DigitalOcean). Prerequisites: Docker installed, PostgreSQL available. You can reuse the Railway PostgreSQL as external DB.',dc:'git clone https://github.com/hanoi96international-gif/Aequitas\ncd Aequitas\n\n# Build the image (~3 min)\ndocker build -t aequitas-node .\n\n# First start (NODE_KEY will be printed in logs)\ndocker run -d --name aequitas-node --restart unless-stopped \\\n  -e DATABASE_URL="postgres://user:pass@host:5432/aequitas" \\\n  -e RELAYER_PRIVATE_KEY="0xYOUR_PRIVATE_KEY" \\\n  -e RELAYER_ADDRESS="0xYOUR_NODE_WALLET" \\\n  -e NODE_OPERATOR_WALLET="0xYOUR_HUMAN_WALLET" \\\n  -e PEER_SECRET="get-from-network-operator" \\\n  -e SELF_URL="http://YOUR-SERVER-IP:8080" \\\n  -e PRIMARY_NODE_URL="https://aequitas.digital" \\\n  -e BOOTSTRAP_SNAPSHOT_URL="https://aequitas.digital/api/snapshot" \\\n  -e BOOTSTRAP_SIGNER="0x92cbedec9d348b4762cb9af99500ee6139c5b671" \\\n  -e SNAPSHOT_TOKEN="get-from-network-operator" \\\n  -p 8080:8080 -p 4001:4001 aequitas-node\n\n# Copy NODE_KEY from first-start logs:\ndocker logs aequitas-node 2>&1 | grep "SAVE THIS AS NODE_KEY"\n\n# Restart with NODE_KEY for stable Peer ID:\ndocker stop aequitas-node && docker rm aequitas-node\ndocker run -d --name aequitas-node --restart unless-stopped \\\n  -e NODE_KEY="<base64 from above>" \\\n  ... (all other vars) ...\n\n# Open firewall ports:\nufw allow 8080/tcp  # Required\nufw allow 4001/tcp  # Optional (P2P)',dn:'Port 8080 must be reachable from outside so the primary node can sync your blocks. Port 4001 (P2P) is optional — HTTP sync works without it.',
    ...
    de:{title:'Aequitas Node-Betreiber-Handbuch',sub:'Vollständige Schritt-für-Schritt-Anleitung · Aequitas Chain (Chain ID 1926)',badge:'v1.0 · Open Source · Erlaubnisfrei · Kein Stake erforderlich',
      ...
      s5:'5. Schnellstart — Docker / VPS',d5:'Für VPS oder Cloud-VM (Contabo, Hetzner, DigitalOcean). Voraussetzungen: Docker installiert, PostgreSQL verfügbar. Railway-PostgreSQL kann als externe DB wiederverwendet werden.',dc:'git clone https://github.com/hanoi96international-gif/Aequitas\ncd Aequitas\n\n# Image erstellen (~3 Minuten)\ndocker build -t aequitas-node .\n\n# Erster Start (NODE_KEY wird in Logs ausgegeben)\ndocker run -d --name aequitas-node --restart unless-stopped \\\n  -e DATABASE_URL="postgres://user:pass@host:5432/aequitas" \\\n  -e RELAYER_PRIVATE_KEY="0xDEIN_PRIVATER_SCHLUESSEL" \\\n  -e RELAYER_ADDRESS="0xDEINE_NODE_WALLET" \\\n  -e NODE_OPERATOR_WALLET="0xDEINE_MENSCH_WALLET" \\\n  -e PEER_SECRET="vom-netzwerkbetreiber" \\\n  -e SELF_URL="http://DEINE-SERVER-IP:8080" \\\n  -e PRIMARY_NODE_URL="https://aequitas.digital" \\\n  -e BOOTSTRAP_SNAPSHOT_URL="https://aequitas.digital/api/snapshot" \\\n  -e BOOTSTRAP_SIGNER="0x92cbedec9d348b4762cb9af99500ee6139c5b671" \\\n  -e SNAPSHOT_TOKEN="vom-netzwerkbetreiber" \\\n  -p 8080:8080 -p 4001:4001 aequitas-node\n\n# NODE_KEY aus ersten Start-Logs kopieren:\ndocker logs aequitas-node 2>&1 | grep "SAVE THIS AS NODE_KEY"\n\n# Mit NODE_KEY neu starten (stabile Peer-ID):\ndocker stop aequitas-node && docker rm aequitas-node\ndocker run -d --name aequitas-node --restart unless-stopped \\\n  -e NODE_KEY="<base64 von oben>" \\\n  ... (alle anderen Vars) ...\n\n# Firewall-Ports öffnen:\nufw allow 8080/tcp  # Pflicht\nufw allow 4001/tcp  # Optional (P2P)',dn:'Port 8080 muss von außen erreichbar sein, damit der Primär-Node deine Blöcke syncen kann. Port 4001 (P2P) ist optional — HTTP-Sync funktioniert auch ohne.',
